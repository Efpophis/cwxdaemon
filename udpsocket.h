...
class UdpSocket
{
    public:
        ...
        bool initialize(unsigned short port=0);
        
        int send(const std::string& host, unsigned short port, 
                  unsigned char* msg, uint32_t length);
              
        
        int receive(unsigned char* msg, uint32_t length, unsigned short* port=NULL);
